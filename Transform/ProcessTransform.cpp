...
CommandType ProcessTransform::EvaluteCommand(const std::string& command)
{
	...
	else if (command[0] == '+' || isdigit(command[0]))
	{
	...
	for (size_t idx = 0; idx < data.length(); ++idx)
	{
		...
		if (fndIdx != keylocationNum.end())
		{
			...
			intCmd = abs(intCmd);
 			...
 			if (intCmd > (int)keylocationNum.size())
			{
				val = intCmd % (int)keylocationNum.size();
			}
			else
			{
				val = intCmd;
			...
			val = ((fndIdx->second - val) + (int)keylocationNum.size()) % (int)keylocationNum.size();
			if (val <= 41 && val > 0)
			{
